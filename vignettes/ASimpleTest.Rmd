...
grid = seq(grid_bds[1], grid_bds[2], length.out = 100)
intl = grid[2]-grid[1]
...
Ufun = function(theta, grad=FALSE) U_kappa(theta = theta, init = lik_init, invC = invC,
                                               alpha = prec_alpha, beta = prec_beta, grad = grad)
...
```{r}
res = aMALA(q_cur = theta, u_cur = u, U = Ufun, Mf = Mf, c = 1.2, eps = 0.1)
```

We invoke the `mcmc_sampling` function with `splitHMC` to run MCMC on the data.

```{r results='hide'}
res_MCMC = mcmc_sampling(data = gene, alg = 'aMALA', nsamp = nsamp,
                         nburnin = nburnin)
```
